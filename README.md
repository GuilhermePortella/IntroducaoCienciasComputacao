...
| | Degradação | Operação limitada sob falha em vez de parada total. |
...
| | Consenso distribuído | Algoritmos de coordenação em sistemas distribuídos. |
...
| | Protocolo de status de certificado online (OCSP) | Verificação em tempo real de certificados. |
| | Internet Content Adaptation Protocol (ICAP) | Filtragem e modificação de conteúdo HTTP. |
| | Engenharia de segurança | Planejamento de segurança em sistemas. |
| | Sistema de controles de voo (Fly-by-wire) | Exemplo crítico de sistema seguro. |
| | Gestão de recursos da tripulação (CRM) | Prática em segurança operacional. |
